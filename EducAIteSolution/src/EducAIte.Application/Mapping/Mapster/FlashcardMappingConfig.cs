...
public sealed class FlashcardMappingConfig : IRegister
{
    public void Register(TypeAdapterConfig config)
    {
        config.NewConfig<Flashcard, FlashcardResponse>()
            ...
            .Map(dest => dest.AcceptedAnswerAliases, src => src.AcceptedAnswerAliases
        ...
        config.NewConfig<StudentFlashcard, StudentFlashcardProgressResponse>()
            ...
            .Map(dest => dest.StudentCourseSqid, src => GetSqidService().Encode(GetFlashcard(src).Note.Document.Folder.StudentCourseId))
            ...
            .Map(dest => dest.LastEvaluationVerdict, src => src.LastEvaluationVerdict.HasValue ? src.LastEvaluationVerdict.Value.ToString() : null)
            .Map(dest => dest.LastQualityScore, src => src.LastQualityScore);
        ...
        config.NewConfig<Flashcard, FlashcardReviewItemResponse>()
            ...
            .Map(dest => dest.NoteSqid, src => GetSqidService().Encode(src.NoteId))
            .Map(dest => dest.DocumentSqid, src => GetSqidService().Encode(src.Note.DocumentId))
            .Map(dest => dest.StudentCourseSqid, src => GetSqidService().Encode(src.Note.Document.Folder.StudentCourseId))
